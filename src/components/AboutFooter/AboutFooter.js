...
export default function AboutFooter() {
  ...
  useEffect(() => {
    ...
    const fitLayout = () => {
      ...
      const stackLinks = contentW < 560;
      if (links) {
        links.style.position = stackLinks ? "static" : "absolute";
        links.style.alignSelf = stackLinks ? "flex-end" : "";
        links.style.marginTop = stackLinks ? "0.35rem" : "0";
      }
      ...
      const linksH = stackLinks && links ? links.offsetHeight + 6 : 0;
      const minVideo = Math.max(MIN_VIDEO_PX, contentH * VIDEO_MIN_SHARE);
      const maxHeadlineH = Math.max(
        36,
        contentH - metaH - minVideo - linksH - videoHeadlineGap
      );
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            paddingTop: `calc(var(--mm-nav-bottom, 5.5rem) + 2rem)`,
        ...
        >
          <div ref={frameRef} className="flex flex-col w-full h-full min-h-0">
            ...
            >
              ...
              <ul
                ref={linksRef}
                className="absolute right-0 bottom-0 m-0 p-0 list-none flex flex-col items-end text-right will-change-transform"
