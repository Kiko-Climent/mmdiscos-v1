export const DataReleases = [
  { 
    artist: "mogwaa",
    title: "Hazzy Dreams LP",
    image: "/img3.jpg",
    year: "2023",
    ref: "mmd028",
    tracklist: [
      "Full Bloom",
      "Nacimiento",
      "Soothing",
      "Levitation",
      "Flashback",
      "Dispatching",
      "Illusions",
      "Echoes Of You",
      "Moondance",
      "Footprints",
      "It Always Comes And Goes",
      "Swingin'",
    ],
    bandcamp: "https://mogwaa.bandcamp.com/album/hazy-dreams",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/mogwaa-hazy-dreams-lp-1"
  },
  
  // MM DISCOS FACTORY - MM Discos Factory
  { 
    artist: "Various Artists",
    title: "MM Discos Factory",
    image: "/Factory Edits - cover.jpg",
    year: "2023",
    ref: "mmd024",
    tracklist: [
      "Jaisiel - Timeless",
      "Tech Support - Dream Space",
      "Trepanado - Abrir Fogo",
      "Clap Kent - El Jaguar",
    ],
    bandcamp: "https://mmdiscos.bandcamp.com/album/mm-discos-factory-edits",
    soundcloud: "hhttps://soundcloud.com/mmdiscos/sets/v-a-mm-discos-factory"
  },
  // STATUES - Night Runner
  { 
    artist: "Statues",
    title: "Night Runner",
    image: "/statues.jpeg",
    year: "2024",
    ref: "mmd030",
    tracklist: [
      "Slipstream",
      "Dodge a Bullet",
      "Hawk",
      "Night Runner",
      "Lake Drum"
    ],
    bandcamp: "https://mmdiscos.bandcamp.com/album/night-runner",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/statues-night-runner"
  },
  // CORBEN - Peachland LP
  { 
    artist: "Corben",
    title: "Peachland LP",
    image: "/corben_peachland_cover.jpg", 
    year: "2024",
    ref: "mms007",
    tracklist: [
      "Make It Move",
      "Fire to Flame ",
      "Six Inches",
      "Get Open",
      "Staring Into the Sun",
      "Potion",
      "Precious",
      "Falling Backwards "
    ],
    bandcamp: "https://mmdiscos.bandcamp.com/album/peachland",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/corbeb-peachland-lp-mm-saturns"
  },
  // CELEX - Capa Disco Works
  { 
    artist: "Celex",
    title: "Capa Disco Works",
    image: "/Celex - cover.jpg",
    year: "2024",
    ref: "mmd031",
    tracklist: [
      "Aquarius",
      "Comme ça",
      "Leisure",
      "Tanzen",
      "Maniac Of Palma ",
      "Patrick's Cha Cha "
    ],
    bandcamp: "https://mmdiscos.bandcamp.com/album/capa-disco-works",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/celex-capa-disco-works-mm"
  },
  // DAICHI - Edits Vol.01-02-03
  { 
    artist: "Daichi",
    title: "Edits Vol.01-02-03",
    image: "/Daichi - cover.jpg",
    year: "2024",
    ref: "mmd033",
    tracklist: [
      "See Of Cosmic",
      "Dampness",
      "You Got Me",
      "Aja Aja",
      "The Heart part 5",
      "Groove Wave (Bonus Slow Disco Edit)"
    ],
    bandcamp: "https://daichivy.bandcamp.com/album/edits-vol-01-02-03",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/daichi-edits-vol-01-02-03"
  },

  // UNGARO & PHIL ANKER - 12 hour EP
  { 
    artist: "ungaro & phil anker",
    title: "12 hour EP",
    image: "/img1.jpg",
    year: "2025",
    ref: "mmd035",
    tracklist: [
      "Fantasy",
      "Dance",
      "Thunderstorm",
      "Focus"
    ],
    bandcamp: "https://iamungaro.bandcamp.com/album/12-hours-ep",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/ungaro-phil-anker-12hours-ep"
  },
  // NAIRLESS - Dreamwalker EP
  { 
    artist: "NairLess",
    title: "Dreamwalker EP",
    image: "/img4.jpg",
    year: "2025",
    ref: "mmd036",
    tracklist: [
      "Kyobashi",
      "Dreamwalker",
      "Don't Go",
      "Eternal Sunshine"
    ],
    bandcamp: "https://nairless.bandcamp.com/album/dreamwalker-ep",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/nairless-dreamwalker-ep-mm"
  },
  // PLEASURE VOYAGE - Freedom Spirit EP
  { 
    artist: "Pleasure Voyage",
    title: "Freedom Spirit EP",
    image: "/img5.jpg",
    year: "2025",
    ref: "mmd032",
    tracklist: [
      "Cloud Waves (New Age Balearic Mix)",
      "Surf Meditation ",
      "Verano Espanola",
      "Freedom Spirit",
      "Archipelagos",
      "Cloud Waves (Ambient Meditation Mix)"

    ],
    bandcamp: "https://pleasurevoyage.bandcamp.com/album/freedom-spirit",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/pleasure-voyage-freedom-spirit"
  },
  // GUILLEMUSIC - The Ten9Eight EP
  { 
    artist: "guillaume",
    title: "The Ten9Eight EP",
    image: "/img2.jpg",
    year: "2025",
    ref: "mmd037",
    tracklist: [
      "Ten9Eight",
      "Despertar",
      "G&G feat. Gurrex",
    ],
    bandcamp: "https://guillaumemusic.bandcamp.com/album/the-ten9eight-ep",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/guillaume-the-ten9eight-ep-mm"
  },
  // PONTCHO - Club Solsticio
  { 
    artist: "Pontcho",
    title: "Club Solsticio",
    image: "/MMD038.png",
    year: "2025",
    ref: "mmd038",
    tracklist: [
      "Yes, Maybe No",
      "Dejame En Paz",
      "You Know Those Girls",
      "When I Say Go",
    ],
    bandcamp: "https://mmdiscos.bandcamp.com/album/club-solsticio",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/pontcho-club-solsticio"
  },
  // NIC JALUSI - Eternal Sunset EP
  { 
    artist: "Nic Jalusi",
    artist_name: "Nic Jalusi",
    title: "Eternal Sunset EP",
    image: "/MMD039.png",
    year: "2025",
    ref: "mmd039",
    tracklist: [
      "Sunset '92",
      "Mystic Riddim",
      "Gamballa Acid",
      "La Noche",
      "Eternal Dance",
      "X-Break"
    ],
    bandcamp: "https://nicjalusi.bandcamp.com/album/eternal-sunset-ep",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/nic-jalusi-eternal-sunset-ep"
  },
  // SOCARRAT VOL.1
  { 
    artist: "Various Artists",
    artist_name: "Socarrat vol.1",
    title: "Socarrat vol.1",
    image: "/MMD040_Cover-1.jpg",
    year: "2025",
    ref: "mmd040.1",
    tracklist: [
      "Pleasure Voyage - Puertas del Cielo",
      "Triple Nickel - Moving on Time (Tucan Discos Edit)",
      "Gats - Astra",
      "Mori Ra - Hood Edit",
      "Balearic Eric - Space Vacation",
      "Breezy S - The Jungle",
      "Distance - Sunrise",
      "Nic Jalusi - Summer of Love",
      "Pontcho - Aqui Nao Cola"
    ],
    bandcamp: "https://mmdiscos.bandcamp.com/album/socarrat-vol-1-10-years-of-mm-discos",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/socarrat-volume-1-10-years-of-mm-discos"
  },
  // SOCARRAT VOL.2
  { 
    artist: "Various Artists",
    artist_name: "Socarrat vol.2",
    title: "Socarrat vol.2",
    image: "/MMD040-2.png",
    year: "2025",
    ref: "mmd040.2",
    tracklist: [
      "Feel Fly - Arama Depesco ",
      "Das Komplex - Baby",
      "Ruf Dug - Ay b'y",
      "Saturn - Mood Swinger",
      "Statues - Hollow Winds ",
      "Jarle Bråthen - Everything Will Soon Reveal Itself",
      "Atlantic Brain - Welcome To The Mischgebiet",
      "Voyle - Siren 0718",
      "Apollon Telefax - Punk",
      "Torrello - The Immediate Rush",
      "Tallac - Phantom Limb",
      "Pacific Coliseum - Reading Stars",
      "Keikee - Spacebirds"
    ],
    bandcamp: "https://mmdiscos.bandcamp.com/track/keikee-spacebirds",
    soundcloud: "https://soundcloud.com/mmdiscos/sets/socarrat-vol-2"
  },
  
]