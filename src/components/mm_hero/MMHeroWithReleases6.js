...
const MMHeroWithReleases6 = () => {
  ...
  useEffect(() => {
    ...
    // Clickeable en la imagen activa para abrir detalle
    spotlightImages.forEach((img, i) => {
      img.addEventListener("click", (e) => {
        e.stopPropagation();
        if (detailActiveRef.current) {
          // Si ya hay detalle abierto, cerrarlo
          closeDetail();
        } else {
          // Verificar si la imagen tiene zIndex alto (imagen activa)
          const zIndex = parseInt(window.getComputedStyle(img).zIndex);
          if (zIndex >= 10) {
            // Si es la imagen activa (zIndex >= 10), abrir detalle
  ...
  return (
    <>
      ...
      <section
        ...
        style={{ zIndex: 10 }}
      >
        <div className="spotlight-images absolute inset-0 z-[10]" style={{ pointerEvents: "auto" }}>
          {ReleaseImages.map((item, index) => {
            ...
            return (
              <div key={index}
                ...
                style={{ transform: "translate3d(-50%, 350%, 0)", willChange: "transform", cursor: "pointer" }}
              >
...
export default MMHeroWithReleases6;