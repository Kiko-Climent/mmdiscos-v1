...
/** Miniaturas y hero en el mismo plano Z en modo foco (si z distinta, la perspectiva desplaza y el hero tapa la columna). */
const FOCUS_THUMB_Z = 0;

/** Modo foco v3: columna izquierda + padding desde el borde */
const THUMB_SCALE   = 0.12;
/** Hueco horizontal mundo entre borde derecho de miniaturas y borde izquierdo del hero (proporcional al ancho del hero). */
...
/** Entrada modo foco: miniaturas en cascada (no todas a la vez). */
...
export default function ReleasesSlider3() {
  ...
  useEffect(() => {
    ...
    // Posiciones en modo foco: columna izquierda + hero al lado
    function getLeftColumnAndHero(focusedIndex) {
      const vFov = (FOV * Math.PI) / 180;
      const camZ = camera.position.z;
      const halfW = Math.tan(vFov / 2) * camera.aspect * camZ;

      // Borde izquierdo visible ≈ -halfW; padding solo desde el borde (sin desplazar de más)
