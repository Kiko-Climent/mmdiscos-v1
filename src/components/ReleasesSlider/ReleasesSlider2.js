...
/** Miniaturas en modo foco: Z más atrás; la elegida se pinta encima con renderOrder. */
const FOCUS_THUMB_Z = -140;
...
export default function ReleasesSlider2() {
  ...
  useEffect(() => {
    ...
    // Modo “una carta grande”: el índice centrado puede no ser CENTER_IDX tras hacer scroll.
    let focusMode           = false;
    let focusTransitioning  = false;
    let focusedIndex        = -1;
    let focusScroll         = 0;

    function getPhase1Row() {
      const FILA_SCALE   = 0.12;
      const FILA_GAP     = CARD_W * FILA_SCALE + 6;
      const FILA_START_X = -(SLIDE_COUNT / 2 - 0.5) * FILA_GAP;
      return { FILA_SCALE, FILA_GAP, FILA_START_X };
    }

    function layoutFocusedHero(scroll, heroIdx) {
      const mesh = meshes[heroIdx];
      const offset = wrapCarouselOffset(heroIdx * SPACING - scroll, trackWidth);
      const absDist = Math.abs(offset);
      const t       = Math.min(absDist / (W * 1.1), 1.0);
      const tEased  = Math.pow(t, 0.75);
    ...
    function enterFocus() {
      ...
      focusScroll    = scrollCurrent;

      const { FILA_SCALE, FILA_START_X, FILA_GAP } = getPhase1Row();
      ...
      // WebGL: la carta elegida debe dibujarse al final (equivalente a z-index encima).
      meshes.forEach((m, i) => {
    ...
    function animate() {
      ...
      if (introComplete && !focusTransitioning) {
        if (focusMode) {
          layoutFocusedHero(focusScroll, focusedIndex);
    ...
    const onResize = () => {
      ...
      if (focusMode && !focusTransitioning) {
        ...
        meshes.forEach((mesh, i) => {
          if (i === focusedIndex) return;
          mesh.position.x = FILA_START_X + i * FILA_GAP;
          mesh.position.z = FOCUS_THUMB_Z;
          mesh.scale.set(FILA_SCALE, FILA_SCALE, 1);
        });
      }
    };
