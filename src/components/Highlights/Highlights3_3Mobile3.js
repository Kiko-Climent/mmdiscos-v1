...
// Etiqueta inicial del index durante el intro. Se intercambia por
// SLIDES[0].ref justo al terminar de dibujarse el crossbar (ip → 1),
// usando el mismo fade que el cambio de slide normal.
...
export default function Highlights3_3Mobile_2() {
  ...
  useLayoutEffect(() => {
    ...
    // --hl-mobile-top-pad: valor inicial estático (fallback hasta que el
    // useEffect de abajo lo recalcule midiendo el menu real). Cubre el
    // primer paint del SSR y el rato hasta post-paint del cliente.
    frame.style.setProperty("--hl-mobile-top-pad", `${Math.round(100 * compact)}px`);
    // --hl-mobile-gap eliminada: el layout pasó de flex stack con gap a
    // grid 1fr auto 1fr (cada panel se centra en su mitad), así que el
    // aire ya no se controla con un gap sino con la distribución de las
    // dos filas 1fr. Si más adelante hicieran falta gaps explícitos
    // arriba/abajo del crossbar, se añadiría `rowGap` al style del grid.
  ...
  // ── Top-pad dinámico anclado al bottom del menu global ───────────────
  // Razón: el menu vive en _app.js (padre), su `top` es state que se
  // resuelve en su useLayoutEffect. React commitea layout effects de hijo
  // a padre → el useLayoutEffect de Highlights mediría las pills antes
  // de que _app.js las haya posicionado. useEffect (post-paint) garantiza
  // que ya están donde toca. Patrón idéntico al de AboutFinal2.js.
  //
  // En /releases las pills tienen 2 filas (sub-links: slider / index) →
  // su altura sube ~30px. Sin medición dinámica, los títulos de mobile
  // entrarían en el área de las pills en pantallas pequeñas.
  //
  // NAV_BREATHING_GAP = aire entre el último pill y el primer título.
  // NAV_FLOOR = mínimo defensivo por si el menu no se puede medir.
  useEffect(() => {
    if (window.innerWidth > 900) return;
    const frame = contentFrameRef.current;
    if (!frame) return;

    const NAV_BREATHING_GAP = 16;
    const NAV_FLOOR = 80;

    const updateNavOffset = () => {
      const menu = document.getElementById("mm-global-menu-pills");
      let navBottom = NAV_FLOOR;
      if (menu) {
        const rect = menu.getBoundingClientRect();
        if (rect.bottom > 0) navBottom = Math.max(NAV_FLOOR, rect.bottom);
      }
      frame.style.setProperty(
        "--hl-mobile-top-pad",
        `${Math.round(navBottom + NAV_BREATHING_GAP)}px`
      );
    };
  ...
  useLayoutEffect(() => {
    ...
    // Flag del swap de label al cruzar ip=1. True = HIGHLIGHTS → mmd042
    // ya se ejecutó. Permite también el camino inverso al volver al
    // intro con scroll back, restituyendo "HIGHLIGHTS".
    ...
    const ctx = gsap.context(() => {
      ...
      // ── Phase budget (mobile) ──────────────────────────────────────
      //  1. intro      — listPanel entra por izq, imagePanel por dcha,
      //                  crossbar dibuja de centro hacia los lados.
      //  2. slides     — un viewport por slide.
      //  3. split      — list sale por top, image por bottom (eje Y).
      //  4. videoGrow  — vídeo expande desde el centro.
      //  5. videoRecede — vídeo retrocede + reglas + quote + firma.
      //
      // Intro EN eje X (entrada horizontal) y split EN eje Y (salida
      // vertical). Como son ejes independientes, los transforms de
      // ambas fases se combinan en un único translate3d(Xvw, Yvh, 0)
      // sin pisarse: durante el intro splitFactor=0 (sólo X activo);
      // durante el split introFactor=0 (sólo Y activo); en slides
      // ambos son 0 (centrado).
      //
      // introRange = vh*0.8 (igual que splitRange) para que intro y
      // outro tengan el mismo tempo de scroll → coherencia rítmica.
      const total = SLIDES.length;
      const vh = window.innerHeight;
      const introRange = vh * 0.8;
      ...
      // Helper: fade-swap del label del index. Misma cinemática que el
      // cambio de slide existente → única fuente de verdad para intro,
      // split-back y cambios de slide.
      ...
      const mainTrigger = ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          // ── Paneles: intro (eje X) + split (eje Y) ──────────────
          // introFactor recorre 1 → 0 durante el intro (panel entra
          // por X). splitFactor recorre 0 → 1 durante el split (panel
          // sale por Y). En slides ambos = 0 → panel centrado.
          //
          // listPanel:  intro entra por izq (-100vw) → 0;
          //             split sale por top (0 → -100vh).
          // imagePanel: intro entra por dcha (+100vw) → 0;
          //             split sale por bottom (0 → +100vh).
          const introEased = easeInOutCubic(ip);
          const splitEased = easeInOutCubic(splitp);
          const introFactor = 1 - introEased;
          const splitFactor = splitEased;

          if (listPanelRef.current) {
            const x = -100 * introFactor;
            const y = -100 * splitFactor;
            listPanelRef.current.style.transform = `translate3d(${x}vw, ${y}vh, 0)`;
          }
          if (imagePanelRef.current) {
            const x = 100 * introFactor;
            const y = 100 * splitFactor;
            imagePanelRef.current.style.transform = `translate3d(${x}vw, ${y}vh, 0)`;
          }

          // ── Crossbar horizontal: intro dibuja + split colapsa ────
          // Espejo proporcional del outro (mismo factor 0.7 que desktop).
          // Intro: inset(0% 50% 0% 50%) → inset(0% 0% 0% 0%) dibujando
          //        del centro hacia los lados.
          // Split: inset(0% 0% 0% 0%) → inset(0% 50% 0% 50%) colapsando
          //        de los lados hacia el centro.
          ...
          // Cruce ip<1 (scroll back): restituye "HIGHLIGHTS".
  ...
  return (
    <div ref={rootRef} className="hl-root w-full bg-white">
      ...
      >
        {/* Layout: posicionamiento absoluto con 3 anclajes independientes.
            · crossbar: top:50% + translate(-50%,-50%) → centro exacto del
              viewport, INMUTABLE frente a paddings, menu height, viewport
              size, etc.
            · listPanel: top: navPad / bottom: 50% → ocupa la mitad superior
              (entre pills y crossbar). Su contenido se centra dentro con
              justify-center → centrado en ese espacio.
            · imagePanel: top: 50% / bottom: bottomPad → ocupa la mitad
              inferior (entre crossbar y fondo). Centrado igual.

            El grid 1fr auto 1fr anterior centraba el crossbar en el área
            POST-padding, no en el viewport → al ser paddings asimétricos
            (top dinámico ~116, bottom 26) el crossbar bajaba ~45px. Aquí
            no: el 50% lo clava al centro real. */}
        <div
          ref={contentFrameRef}
          className="absolute inset-0 px-6 z-[1]"
          style={{
        ...
        >
          <div
            ref={listPanelRef}
            className="hl-panel absolute left-0 right-0 flex flex-col items-center justify-center"
            style={{
              top: "var(--hl-mobile-top-pad, 100px)",
              bottom: "50%",
              gap: "var(--hl-mobile-panel-gap, 16px)",
              // Estado inicial offscreen-izquierda. El primer onUpdate
              // del mainTrigger sobreescribe con el valor correcto.
          ...
          >
            ...
            {/* Counter — arranca como "HIGHLIGHTS" durante el intro,
                se intercambia por SLIDES[0].ref al terminar de dibujarse
                el crossbar (ip=1), y luego cambia por slide. */}
            <div ref={indexRef} className="hl-counter-mobile hl-counter-mobile--ref">
              <span ref={counterRef}>{INTRO_INDEX_LABEL}</span>
            </div>
          </div>

          {/* Crossbar — anclado al centro EXACTO del viewport.
              top:50% sobre el contentFrame (que ocupa el sticky completo)
          ...
          <div
            ...
            style={{
              top: "50%",
              bottom: "var(--hl-mobile-bottom-pad, 26px)",
              gap: "var(--hl-mobile-panel-gap, 16px)",
              // Estado inicial offscreen-derecha. El primer onUpdate
              // del mainTrigger sobreescribe con el valor correcto.
              transform: "translate3d(100vw, 0, 0)",
            }}
          >
