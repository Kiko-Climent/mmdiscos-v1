...
export default function Highlights3_1Desktop() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // ── Reveal timelines ───────────────────────────────────────────
      // Quote: word-level split-from-sides. Mitad izquierda entra desde
      // off-screen-left, mitad derecha desde off-screen-right. Stagger
      // outside-in en cada mitad → las dos palabras del centro son las
      // últimas en aterrizar, "se encuentran" en el medio. El layout
      // final queda centrado intacto (cada word es inline-block dentro
      // de un <p> text-center; el x translate no altera el flow, solo
      // mueve el render).
      //
      // Blur parent-level (cheap: una sola rasterización por frame en
      // vez de N) scrubbed sobre toda la wave para un asiento líquido.
      ...
      const mid = Math.ceil(quoteWordEls.length / 2);
      const leftWordEls = quoteWordEls.slice(0, mid);
      const rightWordEls = quoteWordEls.slice(mid);
      const offX = window.innerWidth * 0.65;

      gsap.set(leftWordEls, {
        x: -offX,
        opacity: 0,
        force3D: true,
      });
      gsap.set(rightWordEls, {
        x: offX,
        opacity: 0,
        force3D: true,
      });
      ...
      const wordDuration = 0.55;
      const wordStagger = 0.028;
      const totalQuoteWave =
        wordDuration +
        Math.max(0, Math.max(leftWordEls.length, rightWordEls.length) - 1) *
          wordStagger;
      ...
      quoteRevealTl
        ...
        .to(
          rightWordEls,
          {
            ...
            duration: wordDuration,
            ease: "expo.out",
            stagger: { each: wordStagger, from: "end" },
            force3D: true,
          },
          0
        )
